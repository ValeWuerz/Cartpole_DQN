   ...
   //torch::optim::Adam optimizer_;  
    ...
    //after : class vars are being filled either with input or with other values like "cpu"
        ...
        q_network(nn::Sequential(
            ...
            nn::Functional(torch::relu),
            nn::Linear(nn::LinearOptions(128, 128).bias(false)),
            nn::Functional(torch::relu),
        ...
        {
        optimizer = std::make_unique<torch::optim::AdamW>(q_network->parameters(), learning_rate);
...
//}
    ...
    //other possible hyperparameters: learning rate(specified manually in optimizer), target_update_frequency(f.e. 10))
    {
        ...
        for (int episode = 1; episode <= num_episodes; episode++) {
            ...
            while (episode_steps < max_steps) {
                ...
                // Choose action
                int action = select_action(state, epsilon, env, epsilon_end, epsilon, epsilon_decay, target_counter);
                //epsilon=decay_epsilon(epsilon,epsilon_decay, epsilon_end, target_counter);
                target_counter++;
                ...
                auto experience = std::make_tuple(state.clone(), action, reward, next_state.clone(), done);
                // Print the tuple
                ...
                //if (target_counter % target_update_frequency_ == 0) { 
            ...
            std::cout << "Episode " << episode << "   -   Rewards: "<< episode_reward<< std::endl;
           
        }
    }
     int select_action(Tensor state, float epsilon, CartPoleEnv env, float epsilon_end_, float epsilon_start_, float epsilon_decay_, int target_counter) {
    
    Tensor q_values = q_network->forward(state);
    //cout << "Q_VALUES:  " << endl;
    //cout << q_values << endl;
    int action;
    double eps_threshold = epsilon_end_ + (epsilon_start_ - epsilon_end_) * std::exp(-1. * steps_done / epsilon_decay_);
    steps_done += 1;
    ...
    if (torch::rand({1}).item<float>() < eps_threshold) {
    ...
    cout << next_state_tensor.sizes()<<endl;
 ...
 void update_target_network() {
    //std::shared_ptr<torch::nn::Sequential> q_net_copy = std::make_shared<torch::nn::Sequential>(q_network);
        ...
        auto q_params = q_network->parameters();
        auto target_params = target_network->parameters();
        for (size_t i = 0; i < target_params.size(); ++i) {
            target_params[i].data().mul_(1 - tau);
            target_params[i].data().add_(q_params[i].data() * tau);
}
                //torch::serialize::OutputArchive output_archive;
                //q_network->save(output_archive);
                ////output_archive.save_to("main_net_params.pt");
//
                //torch::serialize::InputArchive input_archive;
                //input_archive.load_from("main_net_params.pt");
                //target_network->load(input_archive);

    }
//float decay_epsilon(float epsilon_, float epsilon_decay_, float epsilon_end_, int target_counter) {
//    //if (epsilon_ > epsilon_end_) {
//    //    epsilon_ *= epsilon_decay_;
//    //    epsilon_ = std::max(epsilon_, epsilon_end_);
//    //}
//    double eps_threshold = epsilon_end_ + (epsilon_ - epsilon_end_) * std::exp(-1. * target_counter / epsilon_decay_);
...
//  
    ...
    int target_update_frequency=200;
