...
public:
    ...
    Tensor reset();
    std::tuple<Tensor, float, int> step(float action);
...
torch::Tensor vectorToTensor(std::vector<float> vec) {
  // Create a tensor options object with the appropriate data type
  torch::TensorOptions options(torch::kDouble);

  // Create a tensor of the appropriate shape and data type
  torch::Tensor tensor = torch::empty({4}, options);

  // Copy the data from the vector to the tensor using a for loop
...
Tensor CartPoleEnv::reset() {
    ...
    m_done = false;
    m_reward = 0.0;
    vector<float>state_vec={m_state[0],m_state[1],m_state[2],m_state[3]};
  torch::Tensor observations_t = vectorToTensor(state_vec);
    return observations_t;
}

std::tuple<Tensor, float, int> CartPoleEnv::step(float action) {
    ...
    vector<float>state_vec={m_state[0],m_state[1],m_state[2],m_state[3]};
  torch::Tensor observations_t = vectorToTensor(state_vec);
    auto state = make_tuple(observations_t, m_reward, m_done);
